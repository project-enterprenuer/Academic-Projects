...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_spvonly_process_check_sum_call( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_spvonly_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_spvonly_autoinheritance_info( int nlhs, mxArray * plhs[],
    int nrhs, const mxArray * prhs[] );
  if (sf_spvonly_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_spvonly_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "spvonly") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_spvonly_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_spvonly_method_dispatcher(SimStruct *S, unsigned int
...
{
  if (!strcmp(machineName,"spvonly")) {
    return(sf_spvonly_method_dispatcher(simstructPtr,chartFileNumber,method,data));
  }

  return 0;
}

extern void spvonly_terminator(void);
...
extern void spvonly_initializer(void);
extern void spvonly_register_exported_symbols(SimStruct* S);
extern void spvonly_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  spvonly_register_exported_symbols(S);
}
